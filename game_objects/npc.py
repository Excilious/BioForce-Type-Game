...
class NPC(GameObject):
    def __init__(self, level_map, tex_id, x, z):
        ...
        self.project = Projectile(level_map,ID.SUPERORB,x,z)
        self.projdir = glm.vec3(0.0)
        #
    ...
    def update(self):
        ...
        #
        self.project.update(self.projdir,self.pos)
        self.animate()
        # set current texture
        self.tex_id = self.state_tex_id + self.frame

    def get_damage(self):
        damage = WEAPON_SETTINGS[self.player.weapon_id]['damage']
        if (self.eng.player.overcharge):
            damage *= 2
        self.health -= damage
        self.is_hurt = True
        #
    ...
    def attack(self):
        ...
        if self.eng.ray_casting.run(start_pos=self.pos, direction=dir_to_player):
            ...
            if random.random() < self.hit_probability and not self.eng.player.godmode:
    ...
    def move_to_player(self):
        ...
        if self.tile_pos in door_map:
            ...
            if (door.tex_id != ID.REDKEY_DOOR and door.tex_id != ID.YELLOWKEY_DOOR and door.tex_id != ID.BLUEKEY_DOOR):
