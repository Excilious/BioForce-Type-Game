...
class ShaderProgram:
    ...
    def update(self):
        ...
        if (self.eng.player.godmode):
            colour = glm.vec3(1.0,0.5,0.5)
        if (self.eng.player.overcharge):
            colour = glm.vec3(0.5,0.5,1.0)
        #self.level['extralight'].write(colour)
        self.level['light_pos'].write(self.player.position)
        self.level['light_color'].write(glm.vec3(1.0,1.0,1.0))
        self.instanced_door['extralight'].write(colour)
        self.instanced_billboard['m_view'].write(colour)
